...
class HomeFragment : Fragment(R.layout.fragment_home) {
    ...
    private fun setupUI() {
        val firstname = prefs.getString(com.dgv6.app.util.Constants.KEY_FIRSTNAME)
        binding.tvGreeting.text = "Hello, $firstname 👋"
        val balance = prefs.getDouble(com.dgv6.app.util.Constants.KEY_BALANCE)
        ...
        binding.btnToggleBalance.setOnClickListener {
            ...
            binding.tvBalance.text = if (balanceVisible)
                prefs.getDouble(com.dgv6.app.util.Constants.KEY_BALANCE).toNaira()
            else "₦ ••••••"
        }
    }
    ...
    private fun observeData() {
        ...
        viewModel.profile.observe(viewLifecycleOwner) { result ->
            if (result is ApiResult.Success) {
                ...
                prefs.saveDouble(com.dgv6.app.util.Constants.KEY_BALANCE, user.balance)
