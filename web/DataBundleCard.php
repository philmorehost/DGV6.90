            ...
            if(userBalance(1) >= $total_price){
                ...
                if($debit === "success"){
                    ...
                    for($i = 0; $i < $quantity; $i++){
                        ...
                        mysqli_query($connection_server, "INSERT INTO sas_databundle_cards (vendor_id, user_id, product_id, service_type, data_type, network, plan_name, validity, price, epin, serial_number, sms_number, status, batch_reference, date_sold) VALUES ('".$get_logged_user_details["vendor_id"]."', '".$get_logged_user_details["id"]."', '".$get_plan['product_id']."', '".$get_plan['service_type']."', '".$get_plan['data_type']."', '".$get_plan['product_name']."', '".$get_plan['plan_code']."', '".$get_plan['validity_days']."', '$unit_price', '$epin', '$sn', '$sms_number', 'Sold', '$batch_reference', CURRENT_TIMESTAMP)");
                    }
