    ...
    if (!empty($customer_id_or_code)) {
        ...
        if ($account_res && $account_res["status"] == "success") {
            ...
            if (isset($account_data["data"]["bank"])) {
                ...
                addUserVirtualBank($acc["id"], "Paystack", $acc["bank"]["name"], $acc["account_number"], $acc["account_name"]);
        ...
        if ($token_res["status"] == "success") {
            ...
            if ($reserve_res["status"] == "success") {
                ...
                foreach($m_data["responseBody"]["accounts"] as $acc) {
                    addUserVirtualBank($acc_ref, $acc["bankCode"], $acc["bankName"], $acc["accountNumber"], $m_data["responseBody"]["accountName"]);
                }
            ...
            if ($res["status"] == "success") {
                ...
                if (isset($pv_data["banks"])) {
                    foreach($pv_data["banks"] as $acc) {
                        addUserVirtualBank($acc["trackingReference"], $acc["bankCode"], $acc["bankName"], $acc["accountNumber"], $acc["accountName"]);
                    }
