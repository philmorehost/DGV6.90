<?php session_start();
    ...
    if(isset($_POST["buy-addon"])){
        ...
        $addon_config = [
            ...
            'sms_bridge' => ['price_opt' => 'sms_bridge_price', 'col' => 'sms_bridge_ordered', 'label' => 'SMS Bridge APK']
        ];
...
<body>
    ...
    <div class="balance-hero shadow-sm">
        ...
        <div class="row">
          ...
          <div class="col-12 col-lg-4">
            <div class="card shadow-sm border-0 rounded-4 overflow-hidden mb-4 h-100">
              <div class="card-header bg-white py-3 border-0">
                  <h5 class="card-title mb-0 text-center"><i class="bi bi-receipt-cutoff me-2 text-primary"></i>Outstanding Bills</h5>
              </div>
              <div class="card-body p-4">
                <form method="post" action="">
                  <div class="mb-3">
                    <label class="form-label small fw-bold text-muted text-uppercase">Select Bill to Pay</label>
                    <select name="bill-id" class="form-select form-select-lg rounded-3" required>
                        ...
                        if(mysqli_num_rows($get_active_billing_details) >= 1){
                            ...
                            if(mysqli_num_rows($get_paid_bill_details) == 0){
                                echo '<option value="'.$active_billing["id"].'">'.strtoupper($active_billing["bill_type"]).' (₦'.number_format($active_billing["amount"], 2).') - Ends '.date('M j', strtotime($active_billing["ending_date"])).'</option>';
                            }
                            }
                        }
                        ?>
                  ...
                  <button name="pay-bill" type="submit" class="btn btn-outline-primary btn-lg w-100 rounded-3 fw-bold border-2">
                      <i class="bi bi-wallet2 me-2"></i>Settle Bill
                  </button>
                </form>
              </div>
            </div>
                    ...
                    <div class="list-group list-group-flush">
                        <?php
                            $addons = [
                                ...
                                'sms_bridge' => ['label' => 'SMS Bridge APK', 'price' => getSuperAdminOption('sms_bridge_price', '0'), 'ordered' => $get_logged_admin_details['sms_bridge_ordered'], 'icon' => 'bi-chat-left-dots']
                            ];
    ...
    $addon_details = [
        ...
        'sms_bridge' => [
            'title' => 'SMS Bridge APK',
            'article' => 'The SMS Bridge APK is a powerful automation engine designed specifically for the **Print Hub Service**. It eliminates the need for manual transaction processing by acting as a real-time gateway between your incoming SMS notifications and the platform\'s vending engine, ensuring your printing business runs 24/7.',
            'features' => [
