<?php session_start();
    ...
    if(isset($_POST["install-product"])){
        ...
        $product_varieties = array(
            "mtn" => array("500mb_7days","1gb","2gb","3gb","5gb"),
            "airtel" => array(),
            "glo" => array(),
            "9mobile" => array()
        );
        $account_level_table_name_arrays = array("sas_smart_parameter_values", "sas_agent_parameter_values", "sas_api_parameter_values");
        install_product($connection_server, $get_logged_admin_details, "sme-data", "sas_sme_data_status", $products_array, $product_varieties, $account_level_table_name_arrays);
    }

    if(isset($_POST["clear-all-plans"])){
        $vid = $get_logged_admin_details["id"];
        $api_q = mysqli_query($connection_server, "SELECT id FROM sas_apis WHERE vendor_id='$vid' AND api_type='sme-data'");
        $api_ids = [];
        ...
        if(!empty($api_ids)){
            ...
            $_SESSION["product_purchase_response"] = "All SME Data plans cleared successfully.";
        } else {
            // Also try clearing by product_id if api_id is empty (fallback)
            $item_name_array = array("mtn", "airtel", "glo", "9mobile");
            foreach($item_name_array as $pname) {
                $pq = mysqli_query($connection_server, "SELECT id FROM sas_products WHERE vendor_id='$vid' AND product_name='$pname' LIMIT 1");
                if($row = mysqli_fetch_assoc($pq)) {
                    $pid = $row['id'];
                    mysqli_query($connection_server, "DELETE FROM sas_smart_parameter_values WHERE vendor_id='$vid' AND product_id='$pid' AND (val_1 LIKE '%mb%' OR val_1 LIKE '%gb%')");
                    mysqli_query($connection_server, "DELETE FROM sas_agent_parameter_values WHERE vendor_id='$vid' AND product_id='$pid' AND (val_1 LIKE '%mb%' OR val_1 LIKE '%gb%')");
                    mysqli_query($connection_server, "DELETE FROM sas_api_parameter_values WHERE vendor_id='$vid' AND product_id='$pid' AND (val_1 LIKE '%mb%' OR val_1 LIKE '%gb%')");
                }
            }
            $_SESSION["product_purchase_response"] = "SME Data plans cleared from pricing table.";
        }
        header("Location: SmeData.php");
        exit();
    }
...
<body>
    ...
    <div class="pagetitle">
      ...
      <nav>
        <ol class="breadcrumb">
          ...
          <li class="breadcrumb-item active">Sme Data</li>
        </ol>
      </nav>
    </div><!-- End Page Title -->
        ...
        <div class="col-12">
            <div class="card shadow-sm border-0 rounded-4 overflow-hidden mb-4">
                <div class="card-header bg-white py-3 border-0 d-flex justify-content-between align-items-center">
                    <h5 class="card-title mb-0"><i class="bi bi-cloud-arrow-down me-2 text-primary"></i>SME Data Package Plan Variation Fetcher</h5>
                    ...
                    <div class="collapse mb-3" id="fetcherGuide">
                        <div class="alert alert-info border-0 shadow-sm rounded-4 mb-0">
                            ...
                            <ol class="small mb-0 text-dark">
                                ...
                                <li>Enter your desired <strong>Markup</strong> percentages (e.g., set Smart % to 5 to add a 5% markup).</li>
                                <li>Click <strong>Fetch Plans</strong> to load the latest available SME packages from the provider.</li>
        ...
        <div class="col-lg-4">
          <div class="card shadow-sm border-0 rounded-4 mb-4">
            ...
            <div class="card-body p-4 text-center">
                <p class="small text-muted mb-4">Wipe all SME Data plans from the pricing table. Use this when switching API providers.</p>
                <form method="post" action="" onsubmit="return confirm('Are you sure you want to clear ALL SME Data pricing plans? This cannot be undone.');">
                    <button name="clear-all-plans" type="submit" class="btn btn-outline-danger w-100 rounded-3 fw-bold">
                        <i class="bi bi-eraser-fill me-2"></i>Clear All SME Plans
                    </button>
                </form>
            </div>
          </div>
        ...
        <div class="col-lg-8">
                            ...
                            if(mysqli_num_rows($select_item_lists) >= 1){
                                while($list_details = mysqli_fetch_assoc($select_item_lists)){
                                    ...
                                    <tr>
                                        ...
                                        <td>'.$status_badge.'</td>
                                        <td class="pe-4 text-end">'.render_action_buttons($list_details["product_name"], "sme-data", $list_details["status"]).'</td>
                                    </tr>';
                                }
                            }
                        ?>
            ...
            <div class="card shadow-sm border-0 rounded-4 overflow-hidden mb-4">
                <div class="card-header bg-white py-3 border-bottom d-flex justify-content-between align-items-center">
                    <h6 class="fw-bold mb-0 text-primary">SME Data Package Pricing (₦)</h6>
                                        ...
                                        if(mysqli_num_rows($product_smart_table) > 0){
                                            while(($product_smart_details = mysqli_fetch_assoc($product_smart_table)) && ($product_agent_details = mysqli_fetch_assoc($product_agent_table)) && ($product_api_details = mysqli_fetch_assoc($product_api_table))){
                                                ...
                                                <tr>
                                                    ...
                                                    <td><input id="'.strtolower($products).'_sme_data_'.str_replace(["_","-"],"_",$product_smart_details["val_1"]).'_smart_level" name="smart-price[]" type="number" step="0.01" value="'.$product_smart_details["val_2"].'" class="form-control form-control-sm text-center product-price" style="max-width:90px"></td>
                                                    <td><input id="'.strtolower($products).'_sme_data_'.str_replace(["_","-"],"_",$product_smart_details["val_1"]).'_agent_level" name="agent-price[]" type="number" step="0.01" value="'.$product_agent_details["val_2"].'" class="form-control form-control-sm text-center product-price" style="max-width:90px"></td>
                                                    <td><input id="'.strtolower($products).'_sme_data_'.str_replace(["_","-"],"_",$product_smart_details["val_1"]).'_api_level" name="api-price[]" type="number" step="0.01" value="'.$product_api_details["val_2"].'" class="form-control form-control-sm text-center product-price" style="max-width:90px"></td>
                                                    <td><input id="'.strtolower($products).'_sme_data_'.str_replace(["_","-"],"_",$product_smart_details["val_1"]).'_days" name="product-days[]" type="number" value="'.$product_api_details["val_3"].'" class="form-control form-control-sm text-center" style="max-width:60px"></td>
                                                    <td>'.$status_badge.'</td>
                                                    <td class="text-end pe-3">
                                                        <div class="btn-group btn-group-sm">
                                                            '.(($product_smart_details['status'] == 1) ?
                                                            '<a href="SmeData.php?action=disable&product_id='.$product_smart_details["product_id"].'&val_1='.$product_smart_details["val_1"].'&api_id='.$product_smart_details["api_id"].'" class="btn btn-outline-warning" title="Disable"><i class="bi bi-pause-fill"></i></a>' :
                                                            '<a href="SmeData.php?action=enable&product_id='.$product_smart_details["product_id"].'&val_1='.$product_smart_details["val_1"].'&api_id='.$product_smart_details["api_id"].'" class="btn btn-outline-success" title="Enable"><i class="bi bi-play-fill"></i></a>'
                                                            ).'
                                                            <a href="SmeData.php?action=delete&product_id='.$product_smart_details["product_id"].'&val_1='.$product_smart_details["val_1"].'&api_id='.$product_smart_details["api_id"].'" class="btn btn-outline-danger" onclick="return confirm(\'Delete this package?\')" title="Delete"><i class="bi bi-trash"></i></a>
                                                        </div>
                                                    </td>
                                                </tr>';
                                                $csv_price_level_array[] = strtolower($products).'_sme_data_'.str_replace(["_","-"],"_",$product_smart_details["val_1"]).",".$product_smart_details["val_2"].",".$product_agent_details["val_2"].",".$product_api_details["val_2"].",".$product_api_details["val_3"];
                                            }
                                        }
                                    }
                                ?>
            ...
            <div class="card shadow-sm border-0 rounded-4 mb-4">
                <div class="card-header bg-white py-3 border-bottom d-flex justify-content-between align-items-center">
                    ...
                    <a href="javascript:void(0)" onclick='downloadFile(`<?php echo implode("\n",$csv_price_level_array); ?>`, "sme-data.csv");' class="btn btn-sm btn-light border"><i class="bi bi-download me-1"></i>Sample Template</a>
                </div>
                <div class="card-body p-4">
    ...
    async function fetchVariations() {
        ...
        try {
            const response = await fetch(`ajax-fetch-plans.php?gateway=${encodeURIComponent(gateway)}&network=${encodeURIComponent(network)}&type=sme`);
    ...
    function applySinglePlan(index) {
        ...
        const network = document.getElementById('fetch-network').value;
        const inputId = `${network}_sme_data_${plan.code.toLowerCase().replace(/[\s\-]/g, '_')}_smart_level`;
