<?php session_start();
    ...
    if(isset($_POST["clear-all-plans"])){
        ...
        if(!empty($api_ids)){
            ...
            $_SESSION["product_purchase_response"] = "All Cable plans cleared successfully.";
        } else {
            $_SESSION["product_purchase_response"] = "No Cable API configured to clear plans for.";
        }
        header("Location: Cable.php");
        exit();
    }
    ...
    if(isset($_POST["install-product"])){
        ...
        $product_varieties = array(
            ...
            "showmax" => array("full_3months", "mobile_only_3months", "sports_mobile_only_3months", "full_sports_mobile_only_3months", "mobile_only_6months", "full_6months", "full_sports_mobile_only_6months", "sports_mobile_only_6months", "sports_only", "sports_only_3months", "sports_only_6months"),
        );
        $account_level_table_name_arrays = array("sas_smart_parameter_values", "sas_agent_parameter_values", "sas_api_parameter_values");
        install_product($connection_server, $get_logged_admin_details, "cable", "sas_cable_status", $products_array, $product_varieties, $account_level_table_name_arrays);
    }
    ...
    if(isset($_POST["update-price"])){
        ...
        if(count($api_id_array) == count($product_id_array)){
        	foreach($api_id_array as $index => $api_id){
                ...
                $product_days = (int)($product_days_array[$index] ?? 30);
...
<body>
        ...
        <div class="col-12">
            <div class="card shadow-sm border-0 rounded-4 overflow-hidden mb-4">
                <div class="card-header bg-white py-3 border-0 d-flex justify-content-between align-items-center">
                    <h5 class="card-title mb-0"><i class="bi bi-cloud-arrow-down me-2 text-primary"></i>Cable Package Plan Variation Fetcher</h5>
                ...
                <div class="card-body p-4">
                    <div class="collapse mb-3" id="fetcherGuide">
                        <div class="alert alert-info border-0 shadow-sm rounded-4 mb-0">
                            ...
                            <ol class="small mb-0 text-dark">
                                <li>Select the <strong>Provider</strong> (DSTV, GOTV, STARTIMES, SHOWMAX) and <strong>Gateway</strong>.</li>
                                <li>Enter your desired <strong>Discount/Profit</strong> percentages (e.g., set Smart % to 5 to add a 5% markup).</li>
                    ...
                    <div class="row g-3">
                        <div class="col-md-3">
                            <label class="form-label small fw-bold">Select Provider</label>
        ...
        <div class="col-lg-4">
          <div class="card shadow-sm border-0 rounded-4 mb-4">
            ...
            <div class="card-body p-4 text-center">
                <p class="small text-muted mb-4">Wipe all Cable TV plans from the pricing table. Use this when switching API providers.</p>
                <form method="post" action="" onsubmit="return confirm('Are you sure you want to clear ALL Cable pricing plans? This cannot be undone.');">
                    <button name="clear-all-plans" type="submit" class="btn btn-outline-danger w-100 rounded-3 fw-bold">
                        <i class="bi bi-eraser-fill me-2"></i>Clear All Cable Plans
                    </button>
                </form>
            </div>
          </div>
          ...
          <div class="card shadow-sm border-0 rounded-4">
            ...
            <div class="card-body p-4 text-center">
              <div class="mb-4">
                <button type="button" class="btn btn-info btn-sm w-100 mb-3 rounded-pill text-white fw-bold" onclick="tickProduct(this, 'all', 'api-product-name', 'install-product', 'jpg');">Select All Providers</button>
                <div class="d-flex flex-wrap justify-content-center gap-2">
                    <img alt="Startimes" id="startimes-lg" src="/asset/startimes.jpg" onclick="tickProduct(this, 'startimes', 'api-product-name', 'install-product', 'jpg');" class="rounded-3 border p-1 cursor-pointer" style="width: 60px; height: 60px; object-fit: contain;"/>
                    <img alt="Dstv" id="dstv-lg" src="/asset/dstv.jpg" onclick="tickProduct(this, 'dstv', 'api-product-name', 'install-product', 'jpg');" class="rounded-3 border p-1 cursor-pointer" style="width: 60px; height: 60px; object-fit: contain;"/>
                    <img alt="Gotv" id="gotv-lg" src="/asset/gotv.jpg" onclick="tickProduct(this, 'gotv', 'api-product-name', 'install-product', 'jpg');" class="rounded-3 border p-1 cursor-pointer" style="width: 60px; height: 60px; object-fit: contain;"/>
                    <img alt="ShowMax" id="showmax-lg" src="/asset/showmax.jpg" onclick="tickProduct(this, 'showmax', 'api-product-name', 'install-product', 'jpg');" class="rounded-3 border p-1 cursor-pointer" style="width: 60px; height: 60px; object-fit: contain;"/>
                </div>
              </div>
              <form method="post" action="">
        ...
        <div class="col-lg-8">
            <div class="card shadow-sm border-0 rounded-4 overflow-hidden mb-4">
                <div class="card-header bg-white py-3 border-bottom">
                    <h6 class="fw-bold mb-0 text-primary">Installed Cable Status</h6>
                </div>
                            ...
                            if(mysqli_num_rows($select_item_lists) >= 1){
                                while($list_details = mysqli_fetch_assoc($select_item_lists)){
                                    ...
                                    <tr>
                                        ...
                                        <td>'.$status_badge.'</td>
                                        <td class="pe-4 text-end">'.render_action_buttons($list_details["product_name"], "cable", $list_details["status"]).'</td>
                                    </tr>';
                                }
                            }
                        ?>
            ...
            <div class="card shadow-sm border-0 rounded-4 overflow-hidden mb-4">
                <div class="card-header bg-white py-3 border-bottom d-flex justify-content-between align-items-center">
                    <h6 class="fw-bold mb-0 text-primary">Cable Package Pricing (₦)</h6>
                                        ...
                                        if(mysqli_num_rows($product_smart_table) > 0){
                                            while(($product_smart_details = mysqli_fetch_assoc($product_smart_table)) && ($product_agent_details = mysqli_fetch_assoc($product_agent_table)) && ($product_api_details = mysqli_fetch_assoc($product_api_table))){
                                                ...
                                                <tr>
                                                    ...
                                                    <td><input id="'.strtolower($products).'_'.str_replace(["_","-"],"_",$product_smart_details["val_1"]).'_smart_level" name="smart-price[]" type="number" step="0.01" value="'.$product_smart_details["val_2"].'" class="form-control form-control-sm text-center product-price" style="max-width:90px"></td>
                                                    <td><input id="'.strtolower($products).'_'.str_replace(["_","-"],"_",$product_smart_details["val_1"]).'_agent_level" name="agent-price[]" type="number" step="0.01" value="'.$product_agent_details["val_2"].'" class="form-control form-control-sm text-center product-price" style="max-width:90px"></td>
                                                    <td><input id="'.strtolower($products).'_'.str_replace(["_","-"],"_",$product_smart_details["val_1"]).'_api_level" name="api-price[]" type="number" step="0.01" value="'.$product_api_details["val_2"].'" class="form-control form-control-sm text-center product-price" style="max-width:90px"></td>
                                                    <td><input id="'.strtolower($products).'_'.str_replace(["_","-"],"_",$product_smart_details["val_1"]).'_days" name="product-days[]" type="number" value="'.$product_api_details["val_3"].'" class="form-control form-control-sm text-center" style="max-width:60px"></td>
                                                    <td>'.$status_badge.'</td>
                                                    <td class="text-end pe-3">
                                                        <div class="btn-group btn-group-sm">
                                                            '.(($product_smart_details['status'] == 1) ?
                                                            '<a href="Cable.php?action=disable&product_id='.$product_smart_details["product_id"].'&val_1='.$product_smart_details["val_1"].'&api_id='.$product_smart_details["api_id"].'" class="btn btn-outline-warning" title="Disable"><i class="bi bi-pause-fill"></i></a>' :
                                                            '<a href="Cable.php?action=enable&product_id='.$product_smart_details["product_id"].'&val_1='.$product_smart_details["val_1"].'&api_id='.$product_smart_details["api_id"].'" class="btn btn-outline-success" title="Enable"><i class="bi bi-play-fill"></i></a>'
                                                            ).'
                                                            <a href="Cable.php?action=delete&product_id='.$product_smart_details["product_id"].'&val_1='.$product_smart_details["val_1"].'&api_id='.$product_smart_details["api_id"].'" class="btn btn-outline-danger" onclick="return confirm(\'Delete this package?\')" title="Delete"><i class="bi bi-trash"></i></a>
                                                        </div>
                                                    </td>
                                                </tr>';
                                                $csv_price_level_array[] = strtolower($products).'_'.str_replace(["_","-"],"_",$product_smart_details["val_1"]).",".$product_smart_details["val_2"].",".$product_agent_details["val_2"].",".$product_api_details["val_2"].",".$product_api_details["val_3"];
                                            }
                                        }
                                    }
                                ?>
            ...
            <div class="card shadow-sm border-0 rounded-4 mb-4">
                <div class="card-header bg-white py-3 border-bottom d-flex justify-content-between align-items-center">
                    ...
                    <a href="javascript:void(0)" onclick='downloadFile(`<?php echo implode("\n",$csv_price_level_array); ?>`, "cable.csv");' class="btn btn-sm btn-light border"><i class="bi bi-download me-1"></i>Sample Template</a>
                </div>
                <div class="card-body p-4">
    ...
    async function fetchVariations() {
        ...
        try {
            const response = await fetch(`ajax-fetch-plans.php?gateway=${encodeURIComponent(gateway)}&network=${encodeURIComponent(network)}&type=cable`);
            ...
            fetchedPlans.forEach((plan, index) => {
                ...
                const row = `
                    <tr>
                        ...
                        <td class="text-success fw-bold">₦${profit.toFixed(2)}</td>
            ...
            tbody.innerHTML = `<tr><td colspan="7" class="text-center text-danger py-4"><i class="bi bi-exclamation-triangle fs-2"></i><br>${err.message}</td></tr>`;
        }
    }
    ...
    function applySinglePlan(index) {
        ...
        const network = document.getElementById('fetch-network').value;
        const inputId = `${network}_${plan.code.toLowerCase().replace(/ /g, '_')}_smart_level`.replace(/-/g, '_');
        const input = document.getElementById(inputId);
        if(input) {
            input.value = plan.smartPrice.toFixed(2);
            document.getElementById(inputId.replace('smart_level', 'agent_level')).value = plan.agentPrice.toFixed(2);
            document.getElementById(inputId.replace('smart_level', 'api_level')).value = plan.apiPrice.toFixed(2);
            ...
            alert(`Plan code ${plan.code} not found in current installation for ${network.toUpperCase()}. Install the provider first.`);
        }
    }
