        ...
        if ($token_res["status"] == "success") {
            ...
            if ($reserve_res["status"] == "success") {
                ...
                foreach($m_data["responseBody"]["accounts"] as $acc) {
                    addVendorVirtualBank($acc_ref, $acc["bankCode"], $acc["bankName"], $acc["accountNumber"], $m_data["responseBody"]["accountName"]);
                }
            ...
            if (!empty($get_logged_admin_details['bvn'])) {
                ...
                if ($res["status"] == "success") {
                    ...
                    foreach($pv_data["banks"] as $acc) {
                        addVendorVirtualBank($acc["trackingReference"], $acc["bankCode"], $acc["bankName"], $acc["accountNumber"], $acc["accountName"]);
                    }
