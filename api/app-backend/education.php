...
if (json_last_error() === JSON_ERROR_NONE) {
    ...
    if ($select_vendor_table) {
        ...
        $education_type_qty = mysqli_real_escape_string($connection_server, trim(strip_tags($decode_post_request["network"])));
        ...
        $education_type_array = array("waec", "neco", "nabteb", "jamb");

        if (
            !empty($username) &&
            !empty($password) &&
            !empty($education_type) &&
            in_array($education_type, $education_type_array) && 
            !empty($education_qty)
        ) {
            ...
            if (mysqli_num_rows($checkuser) == 1) {
                ...
                if (mysqli_num_rows($checkuser_pass) == 1) {
                    ...
                    $api_file_url = $_SERVER["DOCUMENT_ROOT"] . "/web/api/exam.php";
                    if (file_exists($api_file_url)) {
                        $api_post_info_from_app = array("api_key" => $get_user_info["api_key"], "type" => $education_type, "quantity" => $education_qty);
            ...
            } elseif (empty($education_type)) {
                $status_msg = "Education Type is required";
            } elseif (!in_array($education_type, $education_type_array)) {
                $status_msg = "Invalid Education Type";
            } elseif (empty($education_qty)) {
                $status_msg = "Education Qty is required";
            }
        }
...
// fwrite(fopen("education.json", "a"), $app_json . "\n\n");
