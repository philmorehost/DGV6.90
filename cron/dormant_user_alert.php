...
while ($vendor = mysqli_fetch_assoc($vendors_q)) {
    ...
    $dormant_q = mysqli_query($connection_server, "SELECT u.firstname, u.phone_number, MAX(t.date) as last_tx FROM sas_users u LEFT JOIN sas_transactions t ON t.vendor_id=u.vendor_id AND t.username=u.username AND t.status=1 WHERE u.vendor_id='$vid' AND u.status=1 AND u.phone_number != '' GROUP BY u.id HAVING (last_tx < DATE_SUB(NOW(), INTERVAL 14 DAY)) LIMIT 50");
    ...
    while ($user = mysqli_fetch_assoc($dormant_q)) {
        ...
        if ($ai_up) {
            $prompt  = "Write a friendly WhatsApp re-engagement message for $name on $vname. Max 2 sentences.";
        ...
        if (bc_send_whatsapp($phone, $message, 'dormant_alert')) {
            $sent++;
            echo "  ✅ Sent to $name\n";
        }
        sleep(2);
