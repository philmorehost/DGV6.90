...
if (isset($vendor_account_details) && is_array($vendor_account_details)) {
    ...
    if (isset($connection_server)) {
        $stmt_site = mysqli_prepare($connection_server, "SELECT site_title, apk_download_url FROM sas_site_details WHERE vendor_id = ? LIMIT 1");
        ...
        if ($site_row = mysqli_fetch_assoc($result_site)) {
            $site_title       = $site_row['site_title'];
            $apk_download_url = $site_row['apk_download_url'];
        }
        mysqli_stmt_close($stmt_site);

        // Fetch Header Image
        $template_name_pattern = '%template-1%';
        $stmt_img = mysqli_prepare($connection_server, "SELECT header_image FROM sas_vendor_style_templates WHERE vendor_id = ? AND template_name LIKE ?");
        mysqli_stmt_bind_param($stmt_img, "is", $vendor_account_details["id"], $template_name_pattern);
        mysqli_stmt_execute($stmt_img);
        $result_img = mysqli_stmt_get_result($stmt_img);
        if ($img_row = mysqli_fetch_assoc($result_img)) {
            $header_image_url = $img_row['header_image'];
        }
...
<head>
    ...
    <title><?php echo htmlspecialchars($site_title); ?> — Digital Payments Made Simple</title>
    <meta name="description" content="<?php echo htmlspecialchars($site_title); ?> is a one-stop platform for instant Airtime, Data, Electricity, Cable TV, Crypto, Gift Cards and more. Fast, secure, 24/7.">
    <!-- Open Graph -->
    <meta property="og:title" content="<?php echo htmlspecialchars($site_title); ?>">
    <meta property="og:description" content="Buy Airtime, Data, Electricity tokens, Cable TV, Exam Pins and more — instantly. Join thousands of happy users.">
    ...
    <meta name="twitter:description" content="Digital payments made simple — Airtime, Data, Bills, Crypto & more.">
