...
function resolveVendorID($force_recompute = false) {
    ...
    static $static_vendor_id = null;

    if ($force_recompute) {
        $static_vendor_id = null;
        if (isset($_SESSION)) unset($_SESSION['cached_resolved_vendor_id']);
    }

    if ($static_vendor_id !== null && $static_vendor_id > 0) {
        return $static_vendor_id;
    }

    // Session Cache
    $host_key = 'cached_vid_' . str_replace(['.', ':'], '_', $_SERVER['HTTP_HOST']);
    if (isset($_SESSION[$host_key]) && $_SESSION[$host_key] > 0) {
        $static_vendor_id = (int)$_SESSION[$host_key];
        return $static_vendor_id;
    }

    // Explicit override via global
    if (isset($GLOBALS['vendor_id']) && $GLOBALS['vendor_id'] > 0) {
        $static_vendor_id = (int)$GLOBALS['vendor_id'];
        return $static_vendor_id;
    }

    if (!empty($_SERVER["HTTP_HOST"]) && $connection_server) {
        // Sanitize and extract host without port
        $host = strtolower(preg_replace('/^www\./', '', explode(':', $_SERVER['HTTP_HOST'])[0]));
        $host_esc = mysqli_real_escape_string($connection_server, $host);

        $get_v = mysqli_query($connection_server, "SELECT id FROM sas_vendors WHERE (website_url='$host_esc' OR website_url='www.$host_esc') AND status=1 LIMIT 1");
        if ($get_v && $r = mysqli_fetch_assoc($get_v)) {
            $static_vendor_id = (int)$r['id'];
            if (isset($_SESSION)) {
                $_SESSION[$host_key] = $static_vendor_id;
                $_SESSION['cached_resolved_vendor_id'] = $static_vendor_id;
            }
            return $static_vendor_id;
        }
    }
    ...
    if (isset($GLOBALS['select_vendor_table']['id']) && $GLOBALS['select_vendor_table']['id'] > 0) {
        return (int)$GLOBALS['select_vendor_table']['id'];
    }
...
function isServiceEnabled($name, $vid = null) {
    ...
    if (!isset($static_service_cache[$vid])) {
        ...
        if (isset($_SESSION['service_control_cache']) && isset($_SESSION['service_control_vid']) && $_SESSION['service_control_vid'] == $vid) {
            $static_service_cache[$vid] = $_SESSION['service_control_cache'];
        } else {
            ...
            if ($sc_q && mysqli_num_rows($sc_q) > 0) {
                while ($sc_r = mysqli_fetch_assoc($sc_q)) {
                    $static_service_cache[$vid][$sc_r['service_name']] = (int)$sc_r['status'];
                }
            }
            if (isset($_SESSION)) {
                $_SESSION['service_control_cache'] = $static_service_cache[$vid];
                $_SESSION['service_control_vid'] = $vid;
            }
        }
    }
    ...
    // Default to enabled (1) if not explicitly set in control table
    return (!isset($static_service_cache[$vid][$name]) || $static_service_cache[$vid][$name] == 1);
}
...
function addUserVirtualBank($reference, $bank_code, $bank_name, $account_number, $account_name, $vid = 0, $username = "")
{
	global $connection_server;
	...
	if ($get_logged_user_det && !empty($bank_code) && !empty($account_number) && !empty($account_name)) {
		...
		if (($select_banks !== false) && (mysqli_num_rows($select_banks) == 0)) {
			mysqli_query($connection_server, "INSERT INTO sas_user_banks (vendor_id, username, reference, bank_code, bank_name, account_number, account_name) VALUES ('$v_id', '$u_name', '$reference', '$bank_code', '$bank_name', '$account_number', '$account_name')");
		}
	}
}
...
function getUserVirtualBank($vid = 0, $uname = "")
{
	...
	$select_banks = mysqli_query($connection_server, "SELECT * FROM sas_user_banks WHERE vendor_id='$vid' AND username='$uname'");
...
function addVendorVirtualBank($reference, $bank_code, $bank_name, $account_number, $account_name, $vid = 0)
{
	global $connection_server;
	...
	if ($get_vendor_det && !empty($bank_code) && !empty($account_number) && !empty($account_name)) {
		...
		if (($select_banks == true) && (mysqli_num_rows($select_banks) == 0)) {
			mysqli_query($connection_server, "INSERT INTO sas_vendor_banks (vendor_id, reference, bank_code, bank_name, account_number, account_name) VALUES ('" . $get_vendor_det["id"] . "', '$reference', '$bank_code', '$bank_name', '$account_number', '$account_name')");
		}
	}
}
...
function sendVendorEmail($recipient_email, $email_subject, $email_body)
{
	...
	customBCMailSender('', $recipient_email, $email_subject, $mail_html_body, $mail_headers, $is_background);
	//fwrite(fopen("./email-msg.txt", "a++"), "\n".$recipient_email." || ".strtoupper($email_subject)." || ".$email_body."\n");
}
	...
	if (in_array($mailto_type, array_keys($mailto_array))) {
		...
		if (mysqli_num_rows($select_users) >= 1) {
			while ($each_user = mysqli_fetch_assoc($select_users)) {
				$reg_template_encoded_text_array = array("{firstname}" => $each_user["firstname"], "{lastname}" => $each_user["lastname"], "{username}" => $each_user["username"], "{address}" => $each_user["home_address"], "{email}" => $each_user["email"], "{phone}" => $each_user["phone_number"], "{balance}" => toDecimal($each_user["balance"], 2));
	...
	if (in_array($mailto_type, array_keys($mailto_array))) {
		...
		if (mysqli_num_rows($select_vendors) >= 1) {
			while ($each_vendor = mysqli_fetch_assoc($select_vendors)) {
				$reg_template_encoded_text_array = array("{firstname}" => $each_vendor["firstname"], "{lastname}" => $each_vendor["lastname"], "{address}" => $each_vendor["home_address"], "{email}" => $each_vendor["email"], "{phone}" => $each_vendor["phone_number"], "{website}" => $each_vendor["website_url"]);
				$raw_reg_template_subject = $email_subject;
				$raw_reg_template_body = $email_body;
				foreach ($reg_template_encoded_text_array as $array_key => $array_val) {
					$raw_reg_template_subject = str_replace($array_key, $array_val, $raw_reg_template_subject);
...
function makePayhubRequest($req_method, $parameter_url, $req_body, $vid = null, $is_super = null, $is_withdrawal = false)
{
    ...
    $base_url = getSuperAdminOption('payhub_base_url', 'https://payhub.datagifting.com.ng/');
    ...
    if (!empty($data['metadata'])) {
        ...
        if ($meta) {
            ...
            if (!empty($meta['referrer'])) {
                // Referrer example: https://payhub.datagifting.com.ng/checkout.php?key=...&ref=17734855528636&embed=1
...
function syncPayhubVirtualAccounts($vendor_id, $email, $is_vendor = false, $target_username = "", $is_manual = false) {
    ...
    $save_account = function($acc_data, $ref_override = "") use ($is_vendor, $vendor_id, $target_username, $log) {
        ...
        if ($is_vendor) {
            addVendorVirtualBank($ref, "PayHub", $bname, $accno, $accnm, $vendor_id);
        } elseif (!empty($target_username)) {
            addUserVirtualBank($ref, "PayHub", $bname, $accno, $accnm, $vendor_id, $target_username);
        } else {
            return false;
        }
        return true;
...
function getSuperAdminOption($name, $default = '') {
    global $connection_server;
    ...
    if ($q && $r = mysqli_fetch_assoc($q)) {
        $admin_options_cache[$name] = $r['option_value'];
        if (isset($_SESSION)) $_SESSION['super_admin_options_cache'][$name] = $r['option_value'];
